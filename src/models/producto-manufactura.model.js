...
class ProductoManufacturaModel {
  ...
  async create(producto) {
    ...
    try {
      ...
      // Calcular el costo total basado en los materiales o usar el costo proporcionado
      let costoTotal = Number.parseFloat(costo) || 0 // Usar el costo proporcionado como valor predeterminado

      if (materiales && materiales.length > 0) {
        // Si hay materiales, calcular el costo basado en ellos
        const costoMateriales = materiales.reduce((sum, material) => sum + Number.parseFloat(material.subtotal || 0), 0)

        // Solo usar el costo de materiales si es mayor que cero
        if (costoMateriales > 0) {
          costoTotal = costoMateriales
        }
        // Si el costo de materiales es cero, mantener el costo proporcionado
      }

      console.log("Costo final a guardar:", costoTotal)
  ...
  async update(id, producto) {
    ...
    try {
      ...
      console.log("Costo final a guardar en update:", costoTotal)
